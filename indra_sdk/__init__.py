...
def init(task: str = None, delegation_token: str = None, env_var: str = None, daemon_url: str = "http://localhost:18787", is_serverless: bool = None, oidc_token: str = None, agent_name: str = None):
    """
    ...
    if not is_serverless:
        ...
        resolved_task = task or agent_name or os.getenv("INDRA_AGENT_NAME") or "default_agent_mission"
        ...
        try:
            ...
            with urllib.request.urlopen(req) as response:
                ...
                if status == 200:
                    print(f"[*] Indra SDK: Registered session for PID {pid} (Task: '{resolved_task}')")
                else:
        ...
        resolved_agent_name = agent_name or os.getenv("INDRA_AGENT_NAME") or task or "default_agent_mission"
        resolved_task = task or resolved_agent_name

        # Check for passwordless Vercel OIDC token
        if not oidc_token:
            oidc_token = os.getenv("VERCEL_OIDC_TOKEN")
        if oidc_token:
            del_token = os.getenv(env_var or "INDRA_DELEGATION_TOKEN") or ""
            ...
            data_to_sign = f"{resolved_agent_name}|{fido_id}|{delegation_token}|{jwk_bytes.decode('utf-8')}".encode('utf-8')
